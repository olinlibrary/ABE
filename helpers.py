...
def placeholder_recurring_creation(instance, events_list, event):
    ...
    logging.debug("event_end {}".format(event['end']))
    event_end = datetime.strptime(str(event['end']), "%Y-%m-%d %H:%M:%S.%f")
    event_start = datetime.strptime(str(event['start']), "%Y-%m-%d %H:%M:%S.%f")
